...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        # Delete associated destinations when an account is deleted
...
class IncomingDataView(APIView):
    def post(self, request):
        ...
        # Get the app secret token from the request header
        app_secret_token = request.headers.get('CL-XTOKEN')
        if not app_secret_token:
            return Response({'error': 'Unauthenticated'}, status=status.HTTP_401_UNAUTHORIZED)

        # Identify the account based on the app secret token
        try:
        ...
        # Send the data to the account's destinations
